...
unsigned char AlarmUpperValue = ALARMUPPERDEFINEVALUE;  //报警上限值
unsigned char AlarmLowerValue = ALARMLOWERDEFINEVALUE; //报警下限值
...
{
  static unsigned char  LowerFlag;
  LCDMEM[9] |= BIT7;
    if(level == 3)
    {
        LCDMEM[5] |= BIT4;
...
void DisplayRFRSSILevel(signed char rssi)
{
  if(rssi & 0x80)
  {
    rssi = ((rssi - 256)/2) - 75;
  ...
  else if(rssi >= RSSILEVEL3)
  {
    DisplayRFRSSI(3);
  }
  else if(rssi >= RSSILEVEL2)
  {
    DisplayRFRSSI(2);
  }
  else if(rssi >= RSSILEVEL1)
  {
    DisplayRFRSSI(1);
  }
  else if(rssi >= RSSILEVEL0)
  {
    DisplayRFRSSI(0);
  }
}
